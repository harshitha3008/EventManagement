...
export const fetchEventsByAdmin = async () => {
  try {
    ...
    console.log('Sending request to /api/events/byAdmin with token:', 
      ...
      const response = await fetch('https://mahotsav-backend.vercel.app/api/events/byAdmin', {
      method: 'GET',
      headers: {
...
export const submitEvent = async (formData) => {
  ...
  try {
    ...
    const response = await fetch('https://mahotsav-backend.vercel.app/api/events', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`
      },
